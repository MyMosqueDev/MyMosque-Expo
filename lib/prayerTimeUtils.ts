...
import { JummahTime, PrayerSchedule, PrayerTime } from "./types";
 ...
 */
export const formatPrayerTimes = async (city: string, prayerSchedule: PrayerSchedule, jummahTimes: JummahTime) => {
    if (!prayerSchedule) {
    ...
    try {
        ...
        for (let key in map) {
            if((prayerSchedule.timeMode as Record<string, "static" | "increment">)[key] === 'increment') {
                const incrementValue = (prayerSchedule.incrementValues as Record<string, number>)[key];
                (map as any)[key].iqama = addMinutesToTime((map as any)[key].adhan, incrementValue);
            }
        }
...
export const getNextPrayer = (prayerTimes: PrayerTime) => {
    ...
    if (isFriday && prayerTimes.jummah && Array.isArray(prayerTimes.jummah)) {
        ...
        prayerTimes.jummah.forEach((jummah: any, index: number) => {
            const [hours, minutes] = jummah.iqama.split(":").map(Number);
            prayers.push({ 
